...
with open(annotation_json_file,encoding='utf-8') as f:
    data = json.load(f)
    data = data['images']
labels = [item['concept'] for item in data] 
...
def train_and_save(model,model_transforms,num_epochs,annotation_json_file, image_path_resized,save_path):
    bird_dataset = CustomImageDataset(annotation_json_file, image_path_resized, transform=model_transforms, target_transform=None)
...
#print(train_and_save(*getResNet50_model(),num_epochs,annotation_json_file, image_path_resized,save_path))